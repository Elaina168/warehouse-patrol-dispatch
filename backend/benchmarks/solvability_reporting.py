...
def _raise_write_error(
    ...
    *,
    ...
    message = f"写入可解性差分报告失败: {target_path.resolve()}: {exc}"
...
def write_partial_report(output_dir: Path, report: SolvabilityReport) -> None:
...
def write_final_report(output_dir: Path, report: SolvabilityReport) -> None:
