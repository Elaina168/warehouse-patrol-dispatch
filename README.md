...
- 在线会话运行到任意当前 tick 后，可以通过“机器人接入”面板或 `POST /api/sessions/{sessionId}/robots` 接入新机器人；接入会复用现有分配、A*、时空预约避碰、冲突检测和执行安全门，不需要训练。满足空闲、无携货、无硬锁和仍有其他活动机器人等检查的机器人还可以在“当前机器人管理”中永久退役，退役前路径与事件保留，`reset` 恢复初始场景。
...
本机在 2026-08-28 运行完整检查：前端正式构建通过，前端测试 `236/236`，后端测试 `801 passed, 19 skipped`（共收集 `820` 项）。
...
2026-08-27 的当前验证结果为：前端构建通过、前端测试 `234/234`、后端测试 `801 passed, 19 skipped`（共收集 `820` 项）。
