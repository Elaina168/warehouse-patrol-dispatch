...
- 在线会话运行到任意当前 tick 后，可以通过“机器人接入”面板或 `POST /api/sessions/{sessionId}/robots` 接入新机器人；接入会复用现有分配、A*、时空预约避碰、冲突检测和执行安全门，不需要训练。
...
本机在 2026-08-26 运行完整检查：前端正式构建通过，前端测试 `222/222`，后端测试 `793/793`，另有 `19` 个明确跳过的后端测试。
...
2026-08-26 的当前验证结果为：前端构建通过、前端测试 `222/222`、后端测试 `793/793`，另有 `19` 个明确跳过的后端测试。
