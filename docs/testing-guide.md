...
2. 压力场景扩展：继续增加更大规模、更多随机流和更复杂封锁恢复的回归场景。
3. 算法安全升级：后续在独立分支中评估全规划时域零冲突保证与执行前安全门；二者目前未实现。
...
截至 2026-07-16，优先级已统一为 `0..5`；手工和随机任务使用同一接口、库存校验和 `runtimeTaskCount`；后端会话维护四种权威货架状态；唯一固定场景已升级为 `26 × 16` 真实仓库库存流转地图；主界面已清理动态开关与实验遗留。当前建议先推进机器人能力和压力边界；全规划时域零冲突保证与执行安全门另开分支评估。
