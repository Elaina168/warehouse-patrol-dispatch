...
2. 当前规划质量：固定种子压力场景的预测冲突为零；综合场景中避碰规划的预测冲突少于基线，并在冲突到达实际执行 tick 前通过重规划消解。
...
1. 机器人能力完善：定义任务类型能力并将其加入分配约束；该第三级能力目前未实现。载重、速度、电量、充电和故障状态已参与调度。
2. 真实仓库地图：重做为更接近仓库的货架区、入库流水线、出库流水线、巡检路线、充电桩和主/窄通道布局。
3. 压力场景扩展：继续增加更大规模、更多随机流和更复杂封锁恢复的回归场景。
4. 算法安全升级：后续在独立分支中评估全规划时域零冲突保证与执行前安全门；二者目前未实现。
...
截至 2026-07-14，优先级已统一为 `0..5`，运行时任务使用同一接口和 `runtimeTaskCount`，主界面已清理动态开关与实验遗留。当前建议先推进机器人能力和真实仓库地图；全规划时域零冲突保证与执行安全门另开分支评估。
