...
- 4 台机器人分别配置 `R1=1`、`R2=1`、`R3=2`、`R4=2 tick/格` 的移动耗时；悬浮机器人标识可查看该信息。
...
1. 启动综合调度演示，悬浮四台机器人标识，确认 `R1`、`R2` 显示 `每格耗时 1 tick`，`R3`、`R4` 显示 `每格耗时 2 tick`。
2. 观察 `moveTicks=2` 的机器人开始移动：在中间 tick 内，其图标应继续停留在出发格。
...
- 3S竞赛报告、实验图表和录屏资源独立维护在 `competition/3s/`，默认平台测试不包含3D展示、正式PPT或前端实验面板。
...
- 完整检查通过：前端正式构建成功、前端测试 `219/219`、后端测试 `836 passed, 19 skipped`（共收集 `855` 项）。跳过项不计入通过数。
...
- 实验和校准工具保留为内部诊断与回归能力；小规模可解性差分基准的默认 68-case 运行也已完成 JSON/CSV 交叉核对。当前只保留真实 `oracleSolvedPlannerMiss` 的人工复核入口，尚未批准下一项生产算法工作。
