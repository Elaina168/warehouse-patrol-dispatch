...
- 锁定任务只有在原机器人能力仍匹配时才继续分配给原机器人；不兼容锁不能覆盖能力硬约束。
...
如果场景中根本没有支持任务类型的机器人，运行时任务定义会被拒绝并返回 `422`。如果存在兼容机器人但它当前故障或被运行时封锁阻断，任务可进入队列并通过 `failureDetails` 暴露临时恢复条件。前端随机生成器只在车队能力并集内选择任务类型，但后端仍是最终验证权威。
...
能力相关失败按以下顺序解释：先判断是否存在支持任务类型的机器人，再检查配送载重；随后才判断兼容机器人故障、运行时封锁、锁定和地图可达性。无类型兼容机器人属于永久失败，恢复动作是 `addCapableRobotOrChangeTaskType`；类型兼容但配送载重不足仍沿用增加载重机器人或降低 `demand` 的永久恢复建议。只有兼容且载重合格的故障机器人才能进入 `blockingRobotIds`。恢复时应先恢复不兼容机器人验证失败不变，再恢复唯一兼容机器人触发重规划；运行时封锁则只报告实际阻断兼容机器人的单元。
