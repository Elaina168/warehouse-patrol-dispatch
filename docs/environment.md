...
项目启动脚本把自己创建的前后端进程记录在 `.runtime/dev-processes.json`。清单版本为 `1`，每项保存 `role`、`pid` 和 `startedAtUtc`；停止脚本只处理 PID 与记录的进程启动时间都仍然匹配的条目，不按端口枚举或终止其他进程。身份不匹配或停止失败的条目会保留在清单中，不会因 PID 被复用而误杀无关进程。

启动前会先清理清单中仍匹配的旧进程。若 `8011` 上已有包含 `/api/sessions` 的兼容后端，脚本可以复用它，但不会把该外部进程记入清单或在退出时停止它；不兼容的后端占用或任何 `5174` 占用都会明确报错，要求人工处理，不会通过端口强杀未知进程。
