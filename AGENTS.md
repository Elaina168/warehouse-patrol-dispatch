...
- Backend scenario definition validation checks only the fixed map and static robot eligibility for both base and dynamic tasks; dynamic blocked cells and failed robots, including at `triggerTime = 0`, remain runtime-recoverable conditions used by dispatch failure classification.
...
- Current verification snapshot on 2026-09-04: frontend production build passed, frontend tests `219/219`, backend tests `855 passed, 19 skipped` (874 collected).
...
The scale-and-congestion performance study phase 3 is closed. Its accepted schema-3 evidence has five families and thirteen cases, with 65/65 completed and stable runs, 13 summaries, and zero timeout/error in both the baseline and optimized reports. The optimization is limited to selected-robot copy-on-write with parent/sibling isolation; it changes no business API or frontend. The two local reports and their same-machine copy-count comparison are documented in `docs/algorithm.md`, `docs/experiments.md`, and `docs/testing-guide.md`. This evidence does not establish complete MAPF/CBS, arbitrary-input guarantees, or cross-machine performance.
...
2. Core algorithm module completion - 接近完成
   ...
   - The offline algorithm boundary benchmark is implemented with five deterministic families (`scale`, `density`, `seeded`, `bottleneck`, and `online-pressure`) covering thirteen cases, isolated per-run timeouts, JSON/CSV result reports, and stability summaries. Its current output uses `schemaVersion=3`, records planning forecasts separately from online execution safety evidence, and does not establish complete MAPF or full-horizon zero-conflict guarantees.
   ...
   - The density benchmark performance anomaly has been attributed to a timed A* candidate whose goal remained vertex-reserved for the complete search horizon. In the historical schema-2 introduction stage, the planner began rejecting that candidate before state expansion and recording deterministic planning-work diagnostics while preserving the existing candidate order and result semantics; the current five-family, thirteen-case boundary reports carry those diagnostics in `schemaVersion=3`.
...
8. Experiment and comparison tooling - 稳定基线
   ...
   - The offline algorithm boundary benchmark is implemented as command-line evidence for five families and thirteen deterministic scale, density, seeded, bottleneck, and online-pressure cases. Its schema-3 reports distinguish predicted conflicts from active online conflicts and safety interventions; the six existing experiment APIs remain unchanged.
   - The separate solvability differential command compares the fixed-assignment production planner with a bounded exact joint-state reference. Its five completed-run classes are `agreementSolved`, `oracleSolvedPlannerMiss`, `oracleUnsolvedPlannerNoValidPlan`, `oracleUnsolvedPlannerSolved`, and `oracleLimit`; `timeout` and `error` remain independent outcomes in `schemaVersion=1` `results.json`, `runs.csv`, and `case-summaries.csv` reports.
   ...
   - The accepted scale-and-congestion performance comparison uses baseline `C:\Users\zytx\.codex\worktrees\07aa\summer\output\scale-congestion-performance\baseline\20260904T132339Z` and optimized `C:\Users\zytx\.codex\worktrees\07aa\summer\output\scale-congestion-performance\optimized\20260904T132811Z`: each has 65 completed/stable runs, 13 summaries, zero timeout/error, and JSON/CSV cross-checks. Copy totals are `4,182,195` before and `489,605` after, a reduction of `3,692,590` (`88.293109%`); wall-clock is same-machine evidence only.
