...
2. Core algorithm module completion - 接近完成
   ...
   - Next, diagnose the pressure case's incomplete coverage, two failures, and deadline miss before considering threshold calibration. Only if a future rerun has stable completed `fixed-24` evidence from all three cases and actually produces a candidate envelope should that same-machine envelope be manually reviewed; it remains evidence rather than an automatic recommendation. Do not claim portable cross-machine thresholds, complete MAPF capability, or a global zero-conflict guarantee.
